...
def tensor2imgRGB(tensor, out_type=np.uint8, min_max=(0, 1)):
    '''
...
def b_CPUVar_NN(variable, scale):
    ...
    for i in range(B*C):
        ...
        #re_tensor[i] = to_tensor(resize(img, (H_new, W_new), interpolation=Image.NEAREST))
        img = np.squeeze(tensor_v[i].cpu().numpy())
        img = img[::scale,::scale]
        img = Image.fromarray(img, mode='L')
        re_tensor[i] = to_tensor(img)
    re_tensor_v = re_tensor.view((B, C, H_new, W_new))
    return re_tensor_v
def b_GPUVar_NN(variable, scale):
    ...
    for i in range(B*C):
        ...
        #re_tensor[i] = to_tensor(resize(img, (H_new, W_new), interpolation=Image.NEAREST))
        img = np.squeeze(tensor_view[i].cpu().numpy())
        img = img[::scale,::scale]
        re_tensor[i] = torch.tensor(img).float().view([1, H_new, W_new])
    re_tensor_view = re_tensor.view((B, C, H_new, W_new))
    return re_tensor_view
class USRNetPreprocessing(object):
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        # Down sample
        if self.cuda:
            lr_blured_t = b_GPUVar_NN(hr_blured_var, self.scale)
            #lr_blured_t = b_GPUVar_Bicubic(hr_blured_var, self.scale)
        else:
            lr_blured_t = b_CPUVar_NN(hr_blured_var, self.scale)
            #lr_blured_t = b_GPUVar_Bicubic(hr_blured_var, self.scale)
        
        #img_L = sr.downsample_np(np.squeeze(hr_blured_var.cpu().numpy()).transpose(1,2,0), 4, center=False)
        #lr_blured_t = torch.tensor(img_L.transpose(2,0,1)).float().view([1, 3, 64, 64])
        
        # ff= hr_tensor.detach().cpu().numpy()
